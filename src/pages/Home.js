...
class Home extends Component {
  ...
  addTodo = (todo) => {
    ...
    //determine if a task already exists before performing the 
    //action to add an item to the Todo list

    const alreadyExist = this.state.todos.find(t => t.content === todo.content);
    if(alreadyExist){
      return 
    } else {
  ...
  /*
  delete feature which will remove an item from the Todo list once it is complete 
  (user clicks on checkbox button)
  */
